...
- [`apps/web`](./apps/web) — 可替换侧栏聊天草图（Vite）。走 native OpenAPI，不调用 `/v1`，也不替换 compose 里的 Open WebUI `ui`。
...
npm run dev   # 默认 http://localhost:5173 → runtime http://localhost:8080
```

`VITE_API_BASE` 可覆盖 API 地址。runtime 未起时，页面上打开「本地 Mock（仅本机）」即可点选：CreateAgent → 切换 → 同步 chat。详见 [`apps/web/README.md`](./apps/web/README.md)。

Compose 不变：`ui` 仍是 Open WebUI。本壳用 `npm run dev` 跑，不作为产品双 UI。
